...
static void promoteBuffers(vector<string>& lines){
    eachFunction(lines, [&](size_t h, size_t s, size_t e){
        ...
        for(auto& kv:declLine){ const string& nm=kv.first; string amp="&"+nm; bool buffery=false, bareUse=false; int sz=64;
            for(size_t r=s;r<e;r++){ if(r==kv.second) continue; const string& L=lines[r]; auto m=codeMask(L); size_t p=0;
                ...
                { string pat=nm+"["; size_t ip=0; while((ip=L.find(pat,ip))!=string::npos){ bool lb=ip==0||!isIdent(L[ip-1]); size_t q=ip+pat.size();
                    if(lb){ size_t e2=q; while(e2<L.size()&&isdigit((unsigned char)L[e2]))e2++; if(e2>q&&e2<L.size()&&L[e2]==']'){ long k=strtol(L.substr(q,e2-q).c_str(),0,0); if(k+1>sz&&k<(1<<20))sz=(int)k+1; } } ip=q; } } }
            if(!buffery||bareUse) continue;   // only promote buffers used PURELY as &nm / nm[i] — never type-punned as a scalar
            lines[declLine[nm]] = indentOf(lines[declLine[nm]]) + "char " + nm + "[" + std::to_string(sz) + "];";
